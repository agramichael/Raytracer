...
extern vec4 lightSample[SOFT_SHADOW_SAMPLES];
...
vec3 DirectLight( const Intersection& i ) {
	...
	for( int j = 0; j < SOFT_SHADOW_SAMPLES; j++ ) {
		vec4 dir = glm::normalize(lightSample[j] - i.position);
		...
		float radius = glm::distance(vec3(lightSample[j]), vec3(i.position));
...
void generateLightSample() {
    ...
    for (int i = 1; i < SOFT_SHADOW_SAMPLES; ++i)
    {
        ...
        lightSample[i] = vec4(lightPos[0] + x, lightPos[1] + y, lightPos[2] + z, 1);
    }
}
